...
def mainFCN():
    ...
    while len(currentCarInScene) < trafficJamNum:
        ...
        if (endTime - startTime) > (1 / spawnRate) :
            ...
            newCar = car.Car(screen, lineNumber,carId, initialVelocity = 0.3)
            carId = carId + 1
        ...
        if car_numbers  in [0] :
            ...
            currentVechiles[0].setVelocity(0.5)
        elif car_numbers  > 1:
            for cIndex1 in range(car_numbers-2):
                ...
                for cIndex2 in range(car_numbers-1):
            ...
            for cIndex_1 in range(car_numbers-2):
                for cIndex_2 in range(car_numbers-1):
                    if dist_from_c_list[cIndex_1] < dist_from_c_list[cIndex_2] and max(dist_from_c_list) == dist_from_c_list[cIndex_2] :
                        currentVechiles[cIndex_2].setVelocity(0.6)
                        ...
                        currentVechiles[cIndex_1].setVelocity(0.6)
                        sp = (1/1200)* Error_list[cIndex_1][cIndex_2]
                            #currentVechiles[cIndex_2].setVelocity(sp)


                    elif dist_from_c_list[cIndex_1] == dist_from_c_list[cIndex_2] and max(dist_from_c_list) == dist_from_c_list[cIndex_2] :
                        if currentCarInScene[cIndex_1].getDir() == car.Directions.up_dir:
                            currentVechiles[cIndex_1].setVelocity(0.6)
                        elif currentCarInScene[cIndex_2].getDir() == car.Directions.up_dir:
                            currentVechiles[cIndex_2].setVelocity(0.6)                            
'''''
                        elif dist_from_c_list[cIndex_1] < dist_from_c_list[cIndex_2] and not  max(dist_from_c_list) == dist_from_c_list[cIndex_2] :
                            sp = (1/1200)* Error_list[cIndex_1][cIndex_2]
                            currentVechiles[cIndex_1].setVelocity(sp)
                        ...
                        if dist_from_c_list[cIndex_1] < dist_from_c_list[cIndex_2]:
                            sp = currentVechiles[cIndex_1].getVelocity() - abs(Error_list[cIndex_1][cIndex_2])*(1/1400)
